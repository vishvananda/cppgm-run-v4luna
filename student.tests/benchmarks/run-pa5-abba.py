#!/usr/bin/env python3
"""Run fixed A/A calibration followed by paired PA5 compiler ABBA blocks."""
...
def main():
    ...
    def measure(phase, block, slot, binary):
        file = args.output / f"{phase}-{block:02d}-{slot}.measure"
        observed = run(binary, args.input, args.output / "result.ast", file)
